...
\label{code:Bcc.BM.Test.Configuration}

%if style == newcode
\begin{code}

module Bcc.BM.Test.Configuration (
    tests
  ) where
...
import           Bcc.BM.Data.Configuration
import           Bcc.BM.Configuration.Model (Configuration (..),
...
import qualified Bcc.BM.Data.Aggregated as Agg
import           Bcc.BM.Data.AggregatedKind
import           Bcc.BM.Data.BackendKind
import           Bcc.BM.Data.MonitoringEval
import           Bcc.BM.Data.Output
import           Bcc.BM.Data.Observable (ObservableInstance (..))
import           Bcc.BM.Data.Severity
import           Bcc.BM.Data.SubTrace (SubTrace (..))
import           Bcc.BM.Data.Rotation
...
unitConfigurationParsedRepresentation = do
    ...
    encode repr @?=
        (intercalate "\n"
            ...
            , "      - MonotonicClock"
            , "  mapScribes:"
            , "    tbco.background.process: FileSK::testlog"
            , "    tbco.interesting.value:"
            , "    - StdoutSK::stdout"
            , "    - FileSK::testlog"
            , "  mapAggregatedkinds:"
            , "    tbco.background.process: StatsAK"
            , "    tbco.interesting.value: EwmaAK {alpha = 0.75}"
            , "defaultBackends:"
            , "- KatipBK"
            , "defaultScribes:"
            , "- - StdoutSK"
...
unitConfigurationParsed = do
    ...
    cfgInternal @?= ConfigurationInternal
        ...
        , cgMapSeverity       = HM.fromList [ ("tbco.startup", Debug)
                                            , ("tbco.background.process", Error)
                                            , ("tbco.testing.uncritical", Warning)
                                            ]
        , cgMapSubtrace       = HM.fromList [ ("tbco.benchmarking",
                                                    ObservableTraceSelf [GhcRtsStats, MonotonicClock])
                                            , ("tbco.deadend", NoTrace)
                                            ]
        , cgOptions           = HM.fromList
            [ ("mapSubtrace",
                Object $
                HM.fromList [("tbco.benchmarking",
            ...
            , ("mapSeverity", Object $
                              HM.fromList [("tbco.startup",String "Debug")
                                          ,("tbco.background.process",String "Error")
                                          ,("tbco.testing.uncritical",String "Warning")])
            , ("mapAggregatedkinds", Object $
                                     HM.fromList [("tbco.interesting.value",
                                                        String "EwmaAK {alpha = 0.75}")
                                                 ,("tbco.background.process",
                                                        String "StatsAK")])
            , ("cfokey", Object $ HM.fromList [("value",String "Release-1.0.0")])
            , ("mapScribes", Object $ HM.fromList [("tbco.interesting.value",
                                            Array $ V.fromList [String "StdoutSK::stdout"
                                                               ,String "FileSK::testlog"])
                                         ,("tbco.background.process",String "FileSK::testlog")])
            , ("mapBackends", Object $
                              HM.fromList [("tbco.user.defined",
        ...
        , cgMapBackend        = HM.fromList [ ("tbco.user.defined"
        ...
        , cgMapScribe         = HM.fromList [ ("tbco.interesting.value",
                                                    ["StdoutSK::stdout","FileSK::testlog"])
                                            , ("tbco.background.process", ["FileSK::testlog"])
                                            ]
        , cgMapScribeCache    = HM.fromList [ ("tbco.interesting.value",
                                                    ["StdoutSK::stdout","FileSK::testlog"])
                                            , ("tbco.background.process", ["FileSK::testlog"])
                                            ]
        ...
        , cgMapAggregatedKind = HM.fromList [ ("tbco.interesting.value", EwmaAK {alpha = 0.75} )
                                            , ("tbco.background.process", StatsAK)
                                            ]
