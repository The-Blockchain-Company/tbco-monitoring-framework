...
\subsection{Bcc.BM.Backend.Switchboard}
\label{code:Bcc.BM.Backend.Switchboard}
...
module Bcc.BM.Backend.Switchboard
    (
      Switchboard(..)
    , SwitchboardInternal (..)
    , setSbEKGServer
...
import           Bcc.BM.Configuration.Model (getBackends,
...
import           Bcc.BM.Data.Tracer (Tracer (..))
import qualified Bcc.BM.Backend.Log
import qualified Bcc.BM.Backend.LogBuffer
...
data SwitchboardInternal a = SwitchboardInternal
    ...
    , sbLogBuffer :: !(Bcc.BM.Backend.LogBuffer.LogBuffer a)
    , sbLogBE     :: !(Bcc.BM.Backend.Log.Log a)
...
realizeSwitchboard cfg = do
    ...
    logbuf :: Bcc.BM.Backend.LogBuffer.LogBuffer a <- Bcc.BM.Backend.LogBuffer.realize cfg
    katipBE :: Bcc.BM.Backend.Log.Log a <- Bcc.BM.Backend.Log.realize cfg
                    ...
                    let processItem nli@(LogObject loname _ loitem) = do
                            ...
                            case loitem of
                                ...
                                (Command (DumpBufferedTo bk)) -> do
                                    msgs <- Bcc.BM.Backend.LogBuffer.readBuffer logbuf
    ...
    bs1 <- return (LogBufferBK, MkBackend
                        { bEffectuate = Bcc.BM.Backend.LogBuffer.effectuate logbuf
                        , bUnrealize = Bcc.BM.Backend.LogBuffer.unrealize logbuf
                        })
    bs2 <- return (KatipBK, MkBackend
                        { bEffectuate = Bcc.BM.Backend.Log.effectuate katipBE
                        , bUnrealize = Bcc.BM.Backend.Log.unrealize katipBE
                        })
...
addExternalScribe switchboard sc name =
    withMVar (getSB switchboard) $ \sb ->
        Bcc.BM.Backend.Log.registerScribe (sbLogBE sb) sc name
...
readLogBuffer switchboard = do
    sb <- readMVar (getSB switchboard)
    Bcc.BM.Backend.LogBuffer.readBuffer (sbLogBuffer sb)
