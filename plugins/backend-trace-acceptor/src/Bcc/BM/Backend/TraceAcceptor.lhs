...
\subsection{Bcc.BM.Backend.TraceAcceptor}
\label{module:Bcc.BM.Backend.TraceAcceptor}
...
module Bcc.BM.Backend.TraceAcceptor
    ( TraceAcceptor
    , plugin
    ) where
...
plugin iomgr cf basicTrace _ = getAcceptAt cf >>= \case
  Just acceptors -> do
    ...
    let (cleanups, servers) = unzip socketsNServers
        be :: (Bcc.BM.Backend.TraceAcceptor.TraceAcceptor a)
        be = TraceAcceptor
             { taServers   = servers
