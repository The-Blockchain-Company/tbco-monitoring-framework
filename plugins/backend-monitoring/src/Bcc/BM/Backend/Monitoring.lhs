...
\subsection{Bcc.BM.Backend.Monitoring}
\label{module:Bcc.BM.Backend.Monitoring}
...
module Bcc.BM.Backend.Monitoring
    (
      Monitor
    -- * Plugin
    , plugin
...
import           Bcc.BM.Configuration.Model (Configuration, getMonitors)
...
plugin config trace sb = do
    be :: Bcc.BM.Backend.Monitoring.Monitor a <- realizefrom config trace sb
...
instance FromJSON a => IsBackend Monitor a where
    ...
    realizefrom config sbtrace _ = do
        ...
        monbuf :: Bcc.BM.Backend.LogBuffer.LogBuffer a <- Bcc.BM.Backend.LogBuffer.realize config
