\subsection{Bcc.BM.Backend.Editor}
\label{code:Bcc.BM.Backend.Editor}
...
import           Bcc.BM.Data.BackendKind (BackendKind (EditorBK))
import           Bcc.BM.Data.LogItem
import           Bcc.BM.Data.Output (ScribeId)
import           Bcc.BM.Data.Severity
import           Bcc.BM.Data.SubTrace
import           Bcc.BM.Data.Trace
import           Bcc.BM.Data.Tracer (nullTracer, traceWith)
import           Bcc.BM.Backend.LogBuffer
import           Bcc.BM.Plugin (Plugin (..))
import           Bcc.BM.Rotator (tsformat)
...
plugin config trace sb = do
    be :: Bcc.BM.Backend.Editor.Editor a <- realizefrom config trace sb
...
instance (ToJSON a, FromJSON a) => IsBackend Editor a where
    ...
    realizefrom config sbtrace _ = mdo
        ...
        -- local |LogBuffer|
        logbuf :: Bcc.BM.Backend.LogBuffer.LogBuffer a <- Bcc.BM.Backend.LogBuffer.realize config

        thd <- Async.async $ do
            startGUI defaultConfig { jsPort       = Just port
                                   , jsAddr       = Just "127.0.0.1"
                                   , jsStatic     = Just "tbco-monitoring/static"
...
prepare editor config window = void $ do
    ...
    let exportConfiguration = do
            ...
            let dir = currentDir </> "tbco-monitoring/static/conf"
