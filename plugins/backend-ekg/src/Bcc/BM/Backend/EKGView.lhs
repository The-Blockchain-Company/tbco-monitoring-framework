...
\subsection{Bcc.BM.Backend.EKGView}
\label{code:Bcc.BM.Backend.EKGView}
...
import           Paths_tbco_monitoring (version)

import           Bcc.BM.Backend.ProcessQueue (processQueue)
import           Bcc.BM.Backend.Prometheus (spawnPrometheus)
import           Bcc.BM.Configuration (Configuration, getEKGBindAddr,
...
plugin config trace sb = do
    be :: Bcc.BM.Backend.EKGView.EKGView a <- realizefrom config trace sb
...
instance (ToJSON a, FromJSON a) => IsBackend EKGView a where
    ...
    realizefrom config sbtrace _ = do
        ...
        ekghdl <- getLabel "tbco-monitoring version" ehdl
