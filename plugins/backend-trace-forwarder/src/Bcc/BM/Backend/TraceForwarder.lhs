...
\subsection{Bcc.BM.Backend.TraceForwarder}
\label{module:Bcc.BM.Backend.TraceForwarder}
...
module Bcc.BM.Backend.TraceForwarder
    ( TraceForwarder (..)
    -- * Plugin
    , plugin
    ) where
...
import           Bcc.BM.Data.Configuration (RemoteAddr(..))
import           Bcc.BM.Data.LogItem (LOContent (..), LOMeta (..), LogObject (..))
import           Bcc.BM.Data.Severity (Severity (..))
import           Bcc.BM.IOManager
import           Bcc.BM.Plugin
import qualified Bcc.BM.Snocket as Snocket
...
plugin config _trace _sb tfid getStateDigest = do
    ...
    be :: Bcc.BM.Backend.TraceForwarder.TraceForwarder a <- realize config
