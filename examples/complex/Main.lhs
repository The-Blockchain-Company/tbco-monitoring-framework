...
import           Bcc.BM.Backend.Aggregation
import           Bcc.BM.Backend.Editor
import           Bcc.BM.Backend.EKGView
import           Bcc.BM.Backend.Monitoring
import           Bcc.BM.Backend.Switchboard (Switchboard, readLogBuffer)
import           Bcc.BM.Backend.TraceForwarder
#ifdef LINUX
import           Bcc.BM.Scribe.Systemd
#endif
...
import           Bcc.BM.Data.Configuration (Endpoint(..), RemoteAddr(..))
...
import           Bcc.BM.Observer.Monadic (bracketObserveIO)
#ifdef RUN_ProcObseverSTM
...
main = do
    ...
    -- load plugins
    Bcc.BM.Backend.Editor.plugin c tr sb
      >>= loadPlugin sb
    Bcc.BM.Backend.EKGView.plugin c tr sb
      >>= loadPlugin sb
    forwardTo <- CM.getForwardTo c
    when (isJust forwardTo) $
      Bcc.BM.Backend.TraceForwarder.plugin c tr sb "forwarderMinSeverity" (return [])
        >>= loadPlugin sb
    Bcc.BM.Backend.Aggregation.plugin c tr sb
      >>= loadPlugin sb
    Bcc.BM.Backend.Monitoring.plugin c tr sb
      >>= loadPlugin sb
#ifdef LINUX
    -- inspect logs with 'journalctl -t example-complex'
    Bcc.BM.Scribe.Systemd.plugin c tr sb "example-complex"
      >>= loadPlugin sb
#endif
